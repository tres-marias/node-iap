...
function parseResult(result) {
    ...
    let expirationDate = parseTime(getReceiptFieldValue(result.receipt, "expires_date_ms") || getReceiptFieldValue(result.receipt, "expires_date"));
    ...
    if (result.hasOwnProperty("latest_receipt_info")) {
        if (Array.isArray(result.latest_receipt_info) && result.latest_receipt_info.length > 0) {
            ...
            expirationDate = parseTime(result.latest_receipt_info.expires_date_ms || result.latest_receipt_info.expires_date);
        }
    }
